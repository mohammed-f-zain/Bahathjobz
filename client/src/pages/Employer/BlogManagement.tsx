...
export function BlogManagements() {
  ...
  const delayedSearch = setTimeout(() => {
    ...
    fetchPosts(); // ensure fetchPosts is called
  }, 700);
  ...
  try {
    ...
    if (statusFilter === 'featured') params.append('status', 'featured');
    if (categoryFilter) params.append('category', categoryFilter);
    if (searchTerm) params.append('search', searchTerm);

    const response = await api.get(`/blog/employer/posts?${params.toString()}`);
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await api.get('/blog/employer/analytics');
  ...
  const handleDelete = async (postId: string) => {
    ...
    try {
      await api.delete(`/blog/employer/posts/${postId}`);
          ...
          <Card key={post.id} className="hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center space-x-2 ml-4">
                ...
                <Link to={`/employer/blog/edit/${post.id}`}>
                  <Button variant="ghost" size="sm">
